...
from cgns.constants import MIMIC_CXR_DATA_DIR
from cgns.constants import MIMIC_CXR_REPORT_DIR

reports_path = MIMIC_CXR_REPORT_DIR / "files"
...
def extract_mimic_text():
    ...
    # get all higher up folders (p10, p11, etc)
    ...
    if len(patient_studies) > 0:
        ...
        with open(output_path / 'MGCA/mimic_cxr_sectioned.csv', 'w') as fp:
        ...
        if no_split:
            ...
            with open(output_path / f'MGCA/mimic_cxr_sections.csv', 'w') as fp:
                csvwriter = csv.writer(fp)
            ...
            while n < len(patient_studies):
                n_fn = n // jmp
                with open(output_path / f'CGNS/mimic_cxr_{n_fn:02d}.csv', 'w') as fp:
                    csvwriter = csv.writer(fp)
...
def section_text(text):
    ...
    Returns a three element tuple: 文本内容、标准化后的章节名称以及在原始文本中的起始索引位置。
