...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        ...
        in_h = input.size(2)  # 7
        in_w = input.size(3)
        stride_h = self.img_size[1] / in_h  # 计算步长 32
        stride_w = self.img_size[0] / in_w
        scaled_anchors = [(a_w / stride_w, a_h / stride_h)
                          for a_w, a_h in self.anchors]  # 缩放锚框尺寸

        prediction = input.view(bs,  self.num_anchors,
                                self.bbox_attrs, in_h, in_w).permute(0, 1, 3, 4, 2).contiguous()  # (bs,18,7,7)->(bs,3,7,7,6)

        # Get outputs  计算预测框的中心坐标
        ...
        if targets is not None:
            ...
            # 这些目标张量用于指导模型如何调整预测以更好地匹配实际的边界框位置、大小、置信度和类别。
            ...
            loss_x = self.bce_loss(x * mask, tx * mask)  # 中心坐标偏差的损失
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        mask = torch.zeros(bs, self.num_anchors, in_h, in_w,
                           requires_grad=False)  # (bs,3,7,7) 正样本掩码。这是一个二进制张量，根据IOU用于指示哪些预测框应该被考虑为正样本（即真正包含了目标对象的预测框）
        noobj_mask = torch.ones(
            bs, self.num_anchors, in_h, in_w, requires_grad=False)  # 负样本掩码. 用于指示哪些预测框不应该被考虑为正样本（即不包含目标对象的预测框）
        tx = torch.zeros(bs, self.num_anchors, in_h, in_w,
                         requires_grad=False)  # tx 和 ty: 这两个张量表示中心坐标的目标偏差.表示的是预测框中心相对于其所属网格单元左上角的相对偏移量。例如，如果一个真实框的中心位于网格单元 (i, j) 的中心偏右 0.3 个网格单位，那么 tx[i, j] 的值就是 0.3。
        ty = torch.zeros(bs, self.num_anchors, in_h, in_w,
                         requires_grad=False)
        tw = torch.zeros(bs, self.num_anchors, in_h, in_w,
                         requires_grad=False)  # tw 和 th: 这两个张量表示宽度和高度的目标偏差。它们表示的是预测框的宽度和高度相对于其对应的锚框的对数比例。例如，如果一个真实框的宽度是其对应锚框宽度的 1.5 倍，那么 tw 的值就是 log(1.5)。
        th = torch.zeros(bs, self.num_anchors, in_h, in_w,
                         requires_grad=False)
        tconf = torch.zeros(bs, self.num_anchors, in_h, in_w,
                            requires_grad=False)  # 置信度目标。用于指示哪些预测框应该具有较高的置信度得分。对于每个正样本（mask 为 1 的位置），tconf 的值为 1；对于每个负样本（noobj_mask 为 1 的位置），tconf 的值为 0。
        tcls = torch.zeros(bs, self.num_anchors, in_h, in_w,
                           self.num_classes, requires_grad=False)  # (bs,3,7,7,1)  # 类别目标。用于指示每个预测框应预测的目标类别。对于每个正样本（mask 为 1 的位置），tcls 在该位置处会有一个类别的一热编码表示，即只有目标类别对应的维度值为 1，其余为 0。

        for b in range(bs):  # 遍历每个样本和每个真实标签
            for t in range(target.shape[1]):
                ...
                # Get grid box indices 计算网格索引 gi 和 gj
                gi = int(gx)
                gj = int(gy)
                # Get shape of gt box
                ...
                anchor_shapes = torch.FloatTensor(np.concatenate((np.zeros((self.num_anchors, 2)),
                                                                  np.array(anchors)), 1))
                # Calculate iou between gt and anchor shapes 计算真实框和锚框的 IOU
