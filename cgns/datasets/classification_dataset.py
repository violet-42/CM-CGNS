...
from cgns.datasets.utils import get_imgs, read_from_dicom
...
class CheXpert5_200_Dataset(BaseImageDataset):
    def __init__(self, split="chexpert_5×200", transform=None,
                 img_type="Frontal", data_pct=1.0, imsize=256):
        ...
        selected_columns = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Pleural Effusion']
        self.labels = self.df[selected_columns].values
        self.path = self.df["Path"].values

    def __getitem__(self, index):
        img_path = self.path[index]
        x = self.get_image(img_path, self.imsize, self.transform)
    ...
    @staticmethod
    def get_image(img_path, imsize, transform=None):
