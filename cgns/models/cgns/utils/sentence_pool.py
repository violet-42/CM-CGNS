...
class SentenceAttentionPool(nn.Module):  # 基于注意力机制的句子级特征池化操作
    ...
    def forward(self, x):
        ...
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (L+1)NC 将输入特征的每一维的平均值与原特征连接，其中 L 为原序列长度 N 加上一个全局特征维度
        if self.pos_embed:
