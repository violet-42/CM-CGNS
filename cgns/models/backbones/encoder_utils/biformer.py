...
from mgca.models.backbones.encoder_utils.ops.bra_legacy import BiLevelRoutingAttention
from mgca.models.backbones.encoder_utils._common import Attention, AttentionLePE, DWConv
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2] * x.shape[3]).permute(2, 0, 1).contiguous()  # NCHW -> (HW)NC (4l9,bs,512)
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC  (50,bs,512)
...
class BiFormer(nn.Module):
    def __init__(self, depth=[2, 2, 8, 2], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models[64,128,256,512]
        ...
        nheads= [dim // head_dim for dim in qk_dims]  # 每个头的维度是32，[2,4,8,16]
        ...
        # Classifier head
        self.head = nn.Linear(embed_dim[-1], num_classes) if num_classes > 0 else nn.Identity()  # (512,100)
    ...
    def forward_features(self, x):
        for i in range(4):
            ...
            x = self.stages[i](x)  # 不改变大小和通道
        x = self.norm(x)
        x = self.pre_logits(x)
        return x
# (bs,3,224,224)->(bs,64,56,56)->(bs,128,28,28)->(bs,256,14,14)->(bs,512,7,7)
    def forward(self, x):
        x = self.forward_features(x)  # (bs,512,7,7)
        x = x.flatten(2).mean(-1)  # (bs,512)
...
######## 创建模型 #########
class BiFormerModel(nn.Module):
    def __init__(self, name=None, pool_method='attention', pretrained=True):
        super().__init__()
        self.name = name
        self.feature_dim = 512  # 编码器输出维度特征，在微调时会使用。
        if self.name == 'biformer_tiny' or self.name == 'biformer_small' or self.name == 'biformer_base':
            ...
            # self.pre_logits=self.encoder.pre_logits
            # self.downsample_layers = self.encoder.downsample_layers
            # self.stages = self.encoder.stages

            if pool_method == 'mean':
                self.pool = lambda x: torch.mean(x, dim=[2, 3])  # 保留了原始张量的所有通道信息，但其高度和宽度减小至原来的一半
            elif pool_method == 'attention':
                if self.name == 'biformer_tiny' or self.name == 'biformer_small':
                    self.atten_pool = AttentionPool2d(spacial_dim=7, embed_dim=512, num_heads=8)
                elif self.name == 'biformer_base':
                    self.atten_pool = AttentionPool2d(spacial_dim=7, embed_dim=768, num_heads=8)
                else:
                    self.atten_pool = AttentionPool2d(spacial_dim=7, embed_dim=2048, num_heads=8)  # resnet,vit
    ...
    def forward(self, x, return_features=False):
        x = self.encoder.forward_features(x)  # (bs,512,7,7)
        local_features = x.contiguous()
        global_features = self.atten_pool(x)  # (bs,512)
        global_features = global_features.contiguous().view(global_features.size(0), -1)
        local_features = local_features.view(local_features.shape[0], -1, local_features.shape[1]).contiguous()  # (bs,512,7,7)->(bs,49,512)
        return global_features, local_features  # (bs,512) (bs,49,512)
